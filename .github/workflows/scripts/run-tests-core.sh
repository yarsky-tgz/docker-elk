...
response="$(curl 'https://elasticsearch:9200/logstash-*/_search?q=message:dockerelk&pretty' -s --resolve "elasticsearch:9200:${ip_es}" --cacert "$es_ca_cert" -u elastic:testpasswd)"
echo "$response"
